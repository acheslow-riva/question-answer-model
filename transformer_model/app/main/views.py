...
def load_traced_model():
    ...
    if not 'traced_model.pt' in direct:
        ...
        current_app.finder.get_node("Reader").inferencer.model.language_model.model.eval()
        traced_model = torch.neuron.trace(current_app.finder.get_node("Reader").inferencer.model.language_model.model, example_inputs=inputs) 
        traced_model.save('app/static/data/language_model/traced_model.pt')
    current_app.finder.get_node("Reader").inferencer.model.language_model.model = torch.jit.load('app/static/data/language_model/traced_model.pt')
    current_app.finder.get_node("Reader").inferencer.model.language_model.model.eval()
